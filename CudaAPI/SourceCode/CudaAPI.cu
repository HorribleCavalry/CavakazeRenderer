...
public:
	__duel__ virtual void Call() override
	{
		printf("Called Child0::Call()!\n");
	}
};

__global__ void kernel(Base* baseDevice)
{
	printf("The following run on DEVICE!\n");
	Base temp(*baseDevice);
	Base* tempPtr = &temp;
	memcpy(baseDevice, tempPtr, sizeof(Base));
	baseDevice->Call();
}

int main()
{
	printf("The following run on Host!\n");

	Base* baseHost = new Base;
	baseHost->id = 10;
	Base* baseDevice;
	cudaMalloc(&baseDevice, sizeof(Base));
	cudaMemcpy(baseDevice, baseHost, sizeof(Base), cudaMemcpyKind::cudaMemcpyHostToDevice);

	baseHost->Call();
	kernel << <1, 1 >> > (baseDevice);
	delete baseHost;
	cudaFree(baseDevice);
}